...
def run_calculation_engine(df, mode):
    """수정된 데이터를 바탕으로 전체 수치를 재계산하고 정렬하는 엔진"""
    temp_df = df.copy()
    ...
    # 정렬 로직 (순서 번호 -> 품목명)
    return temp_df.sort_values(by=['순서', '품목']).reset_index(drop=True)
...
st.data_editor(
    ...
    column_config={
        ...
        "품목": st.column_config.TextColumn("품목"), # 한글 키보드 유지 보정
