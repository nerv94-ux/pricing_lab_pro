...
def process_data(df, mode):
    ...
    # 순서대로 정렬 (대표님 요청: 번호 수정 시 해당 위치로 자동 이동)
    ...
    for _, row in df.iterrows():
        try:
            ...
            # [수정 포인트 1] 판매가 직접 입력 여부 판단 및 역산
            # 이전 상태와 비교하기 위해 세션 데이터와 대조 로직 대신 
            # 현재 행의 수치 평형 관계를 확인하여 역산 우선순위 결정
            ...
            # 이론적 판매가와 실제 입력된 판매가가 다르면 -> 판매가 우선 (마진 역산)
            if actual_price > 0 and abs(actual_price - theo_price) > 1:
...
# [수정 포인트 2] 순서 및 판매가를 직접 수정 가능하게 설정
...
with st.expander("ℹ️ 시스템 작동 가이드"):
    ...
    - **수수료 0%**: 프리셋에서 0을 선택하면 수수료 없는 단가가 산출됩니다.
    """)